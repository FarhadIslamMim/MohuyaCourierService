    ...
    <div class="row">
        <div class="col-xl-12">
            <div class="card">
                ...
                <div class="card-body">
                    <!-- Basic example -->
                    <form action="{{route('payment.merchant.invoice')}}" method="GET">
                        @csrf
                        <div class="row">
                            <div class="col-lg-3">
                        ...
                        @if(count($invoices) > 0)
                            @foreach ($invoices as $invoice)
                                <tr>
                                    ...
                                    <td>{{ $invoice->getPercelDetails->merchant->companyName ?? 'No name' }}</td>
                                    <td> {{ \Carbon\Carbon::parse($invoice->created_at)->format('d M Y g:i:s A') }}</td>
                                    <td>
                                        <a href="{{ route('payment.merchant.invoice.export',encrypt($invoice->id)) }}"
                                            class="btn btn-danger" target="_blank"><i class="fa fa-file-pdf-o"></i></a>

                                        <a href="{{route('payment.merchant.invoice.print',encrypt($invoice->id))}}"
                                           class="btn btn-primary" target="_blank"><i class="fa fa-print" aria-hidden="true"></i></a>

                                        <a href="{{route('payment.merchant.invoice.download',encrypt($invoice->id))}}"
